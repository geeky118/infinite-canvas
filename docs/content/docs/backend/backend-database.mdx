...
| `email` | string | 邮箱，注册时必须是 `REGISTER_EMAIL_DOMAIN`（默认 `qq.com`）下的有效地址 |
...
注册流程发送的邮箱验证码。每次发送都会写入一条新记录，校验通过后通过 `used_at` 标记已使用，避免重复消费。

| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `id` | string | 主键 |
| `email` | string | 接收验证码的邮箱（已规范为小写），索引 |
| `code` | string | 验证码明文，仅在 `used_at` 为空且未过期时有效 |
| `purpose` | string | 验证码用途，当前只支持 `register`，索引 |
| `expires_at` | string | 过期时间（RFC3339），索引 |
| `used_at` | string | 消费时间，未消费时为空 |
| `created_at` | string | 创建时间，索引 |
