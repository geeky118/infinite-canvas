...
- `showImageInfo`：是否在图片节点上显示模型、尺寸等生成信息。
- `systemPrompt`：当前画布项目独立使用的系统提示词，不再来自全局模型配置。
...
- `memory`：当前会话的结构化记忆，文本对话会随最近消息一起发送给文字模型；登录用户会跟随画布项目 JSON 通过 `/api/v1/user-data/canvas` 保存到数据库 `user_data.payload`。
- `archivedAt`：会话归档时间；存在时从当前对话列表隐藏，但仍保存在当前画布项目内。

记忆结构：

```ts
type CanvasAssistantMemory = {
  summary: string;
  preferences: string[];
  facts: string[];
...
- `summary`：当前会话稳定目标和上下文的短摘要。
- `preferences`：用户在本会话表达过、会影响后续回答的偏好。
- `facts`：用户确认过的项目事实、画布上下文或引用过的重要节点。
- `openQuestions`：后续仍需要跟进的问题。
- `turns`：已纳入记忆的对话轮数。
